...
if iscell(arc)
    h = [];
    for i = 1:length(arc)
        h = [h matGeom.geom3d.drawCircleArc3d(arc{i}, varargin{:})]; %#ok<AGROW>
    end
    if nargout > 0
        varargout = {h};
    end
...
curve   = matGeom.geom3d.transformPoint3d(curve, trans);

% draw the curve with specified options
h = matGeom.geom3d.drawPolyline3d(curve, varargin{:});
