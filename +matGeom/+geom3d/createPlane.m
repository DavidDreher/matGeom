...
if length(varargin) == 1
    ...
    if iscell(var)
        ...
        for i=1:length(var)
            plane(i,:) = matGeom.geom3d.createPlane(var{i});
        end
    elseif size(var, 1) >= 3
        ...
        % create plane
        plane = matGeom.geom3d.normalizePlane([p1 v1 v2]);
        return;
    end
...
elseif length(varargin) == 2
    ...
    if size(var, 2) == 2
        ...
        n = matGeom.geom3d.sph2cart2([var ones(size(var, 1))]);
    elseif size(var, 2)==3
        % normal is given by a 3D vector
        n = matGeom.geom3d.normalizeVector3d(var);
    else
    ...
    inds = matGeom.geom3d.vectorNorm3d(cross(n, v0, 2))<1e-14;
...
%     end
    ...
    % create direction vectors
    v1 = matGeom.geom3d.normalizeVector3d(cross(n, v0, 2));
    v2 = -matGeom.geom3d.normalizeVector3d(cross(v1, n, 2));
