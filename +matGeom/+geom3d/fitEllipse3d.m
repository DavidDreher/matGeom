...
tfmPoints = matGeom.geom3d.transformPoint3d(centeredPoints, R');
...
TFM2D = matGeom.geom3d.createRotationOz(phi2D);
TFM2D(1:3,4)=[X; Y; 0];
...
[PHI, THETA, PSI] = matGeom.geom3d.rotation3dToEulerAngles(TFM3D_ROT,'ZYZ');

% Test if psi is correct
TFM3D_test = matGeom.geom3d.eulerAnglesToRotation3d(PHI, THETA, PSI,'ZYZ');
...
if parser.Results.visualization == true
    ...
    % Fitted ellipse
    matGeom.geom3d.drawEllipse3d(fittedEllipse3d)
end
