...
if npl > 1 || npt == 1
    s = dot(bsxfun(@minus, point, p0), d1, 2) ./ matGeom.geom3d.vectorNorm3d(d1);
    t = dot(bsxfun(@minus, point, p0), d2, 2) ./ matGeom.geom3d.vectorNorm3d(d2);
else
    % we have npl == 1 and npt > 1
    d1 = d1 / matGeom.geom3d.vectorNorm3d(d1);
    d2 = d2 / matGeom.geom3d.vectorNorm3d(d2);
    inds = ones(npt,1);
