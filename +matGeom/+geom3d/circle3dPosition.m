...
ori     = matGeom.geom3d.circle3dOrigin(circle);

% normal vector of the supporting plane (cartesian coords)
vn      = matGeom.geom3d.sph2cart2d([theta phi]);

% create plane containing the circle
plane   = matGeom.geom3d.createPlane([xc yc zc], vn);

% find position of point on the circle plane
pp0     = matGeom.geom3d.planePosition(ori,    plane);
pp      = matGeom.geom3d.planePosition(point,  plane);
