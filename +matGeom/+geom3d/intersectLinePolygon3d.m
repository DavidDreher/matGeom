...
inter = matGeom.geom3d.intersectLinePlane(line, plane);

% project all points on reference plane
pts2d = matGeom.geom3d.planePosition(poly, plane);
pInt2d = matGeom.geom3d.planePosition(inter, plane);

% need to check polygon orientation
inside = xor(matGeom.polygons2d.isPointInPolygon(pInt2d, pts2d), matGeom.polygons2d.polygonArea(pts2d) < 0);
