...
for iFace = 1:nFaces
    faceInds = tri(iFace, :);
    plane = matGeom.geom3d.createPlane(pts(faceInds, :));
    
    breadth = max(abs(matGeom.geom3d.distancePointPlane(pts, plane)));
...
refPlane = matGeom.geom3d.createPlane(pts(tri(indMinBreadth, :), :));
pts2d = matGeom.geom3d.planePosition(matGeom.geom3d.projPointOnPlane(pts, refPlane), refPlane);

% compute 2D OOBB for projected points
box2d = matGeom.geom2d.orientedBox(pts2d);
...
theta2d = box2d(5);
rot = matGeom.geom2d.createRotation(matGeom.geom2d.deg2rad(theta2d));
tra = matGeom.geom2d.createTranslation(center2d);
transfo = tra * rot;
markers2d = matGeom.geom2d.transformPoint(markers2d, transfo);

% backprojection to 3D space
markers3d = matGeom.geom3d.planePoint(refPlane, markers2d);

% compute 3D vectors and center
centerProj = markers3d(1,:);
v1n = matGeom.geom3d.normalizeVector3d(markers3d(2,:) - centerProj);
v2n = matGeom.geom3d.normalizeVector3d(markers3d(3,:) - centerProj);

% compute rotation matrix and convert to Euler Angles
v3n = matGeom.geom3d.vectorCross3d(v1n, v2n);
rotMat = [v1n' v2n' v3n'];
boxAngles = matGeom.geom3d.rotation3dToEulerAngles(rotMat);
