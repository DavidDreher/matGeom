...
rotZ    = matGeom.geom3d.createRotationOz(phi);
rotY    = matGeom.geom3d.createRotationOy(theta);
rotX    = matGeom.geom3d.createRotationOx(psi);
...
[x, y, z] = matGeom.geom3d.transformPoint3d(v, trans);


%% Process output
if nargout == 0
    % no output: draw the cuboid
    matGeom.meshes3d.drawMesh([x y z], f, varargin{:});
    
elseif nargout == 1
    % one output: draw the cuboid and return handle 
    varargout{1} = matGeom.meshes3d.drawMesh([x y z], f, varargin{:});
