...
for i = 1:nbSides
    planes(i,1:9) = matGeom.geom3d.normalizePlane(matGeom.geom3d.medianPlane(refPoint, neighbors(i,:)));
end
...
for i = 1:nbSides
    ...
    lines(i,1:6) = matGeom.geom3d.intersectPlanes(planes(i,:), planes(ind2,:));
    intersects(2*i-1:2*i,1:3) = matGeom.geom3d.intersectLineSphere(lines(i,:), sphere);
end
