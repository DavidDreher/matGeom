...
% normalize points
p1  = matGeom.geom3d.normalizeVector3d(p1);
p2  = matGeom.geom3d.normalizeVector3d(p2);
p3  = matGeom.geom3d.normalizeVector3d(p3);

% create the plane tangent to the unit sphere and containing central point
plane = matGeom.geom3d.createPlane(p2, p2);

% project the two other points on the plane
pp1 = matGeom.geom3d.planePosition(matGeom.geom3d.projPointOnPlane(p1, plane), plane);
pp3 = matGeom.geom3d.planePosition(matGeom.geom3d.projPointOnPlane(p3, plane), plane);

% compute angle on the tangent plane
pp2 = zeros(max(size(pp1, 1), size(pp3,1)), 2);
alpha = matGeom.geom2d.angle3Points(pp1, pp2, pp3);
