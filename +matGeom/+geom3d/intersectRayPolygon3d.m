...
plane   = matGeom.geom3d.createPlane(poly(1:3, :));

% intersection of 3D ray with the plane
inter   = matGeom.geom3d.intersectLinePlane(ray, plane);

% project all points on reference plane
pts2d   = matGeom.geom3d.planePosition(matGeom.geom3d.projPointOnPlane(poly, plane), plane);
pInt2d  = matGeom.geom3d.planePosition(matGeom.geom3d.projPointOnPlane(inter, plane), plane);

% need to check polygon orientation
inPoly  = xor(matGeom.polygons2d.isPointInPolygon(pInt2d, pts2d), matGeom.polygons2d.polygonArea(pts2d) < 0);
onRay   = matGeom.geom3d.linePosition3d(inter, ray) >= 0;
inside  = inPoly & onRay;
