...
pts = matGeom.polynomialCurves2d.polynomialCurvePoint(t, xCoef, yCoef);

% compute the matrix which transform points such that axis becomes the
% vertical axis
angle   = matGeom.geom2d.lineAngle(axis);
trans   = matGeom.geom2d.createRotation(axis(1:2), pi/2 - angle);

% transform points
pts = matGeom.geom2d.transformPoint(pts, trans);


%% compute first derivatives

% compute first derivatives of the polynomials
dx  = matGeom.polynomialCurves2d.polynomialDerivate(xCoef);
dy  = matGeom.polynomialCurves2d.polynomialDerivate(yCoef);
...
vect = matGeom.geom2d.transformVector([xp yp], trans);
xp = vect(:,1);
yp = vect(:,2);
...
% compute second derivatives
sx  = matGeom.polynomialCurves2d.polynomialDerivate(dx);
sy  = matGeom.polynomialCurves2d.polynomialDerivate(dy);
...
% transform vectors of first derivatives
vect = matGeom.geom2d.transformVector([xs ys], trans);
xs = vect(:,1);
ys = vect(:,2);
