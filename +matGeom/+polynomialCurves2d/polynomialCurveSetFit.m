...
for i = 1:nBranches
    ...
    [dist, ind0] = matGeom.geom2d.minDistancePoints(points(1, :), nodes); %#ok<*ASGLU>
    [dist, ind1] = matGeom.geom2d.minDistancePoints(points(end, :), nodes);
    
    % add nodes to the curve.
    points = [nodes(ind0,:); points; nodes(ind1,:)]; %#ok<AGROW>
    
    % parametrization of the polyline
    t = matGeom.polygons2d.parametrize(points);
    t = t/max(t);
    
    % fit a polynomial curve to the set of points
    [xc, yc] = matGeom.polynomialCurves2d.polynomialCurveFit(...
        t, points, deg, ...
