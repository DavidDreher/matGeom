...
if isnumeric(poly) && any(isnan(poly(:)))
    poly = matGeom.polygons2d.splitPolygons(poly);
end
...
supportLines = matGeom.geom2d.edgeToLine(edges);
intersects = matGeom.geom2d.intersectLines(line, supportLines, tol);
...
pos = matGeom.geom2d.linePosition(intersects(inds, :), supportLines(inds, :), 'diag');
