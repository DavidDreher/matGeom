...
line = matGeom.geom2d.edgeToLine(edge);

% compute all intersections of supporting line with polygon
[intersects, inds] = matGeom.polygons2d.intersectLinePolygon(line, poly, tol);

% keep only intersection points located on the edge
if ~isempty(intersects)
    pos = matGeom.geom2d.linePosition(intersects, line);
