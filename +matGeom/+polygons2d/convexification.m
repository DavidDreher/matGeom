...
if ~isempty(varargin)>0
    ...
    if size(var, 2)==1
        ...
        h = matGeom.polygons2d.supportFunction(poly, N);
    end
else
...
circ = zeros(N, 4);
for i=1:N
    circ(i, 1:4) = matGeom.geom2d.createDirectedCircle(v(i1(i),:), v(i2(i),:), v(i3(i), :));
end
...
if matGeom.geom2d.distancePoints(co(1,:), co(end, :))<1e-10 && size(co, 1)>1
    co = co(1:end-1,:);
end
