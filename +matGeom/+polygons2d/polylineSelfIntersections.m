...
for iEdge1 = 1:nEdges-1
    ...
    for iEdge2 = iEdge1+2:nEdges
        ...
        inter = matGeom.geom2d.intersectEdges(edge1, edge2, tol);
        ...
        if sum(isfinite(inter)) == 2
            ...
            pos1(ip, 1) = iEdge1 - 1 + matGeom.geom2d.edgePosition(inter, edge1);
            pos2(ip, 1) = iEdge2 - 1 + matGeom.geom2d.edgePosition(inter, edge2);
        end
    end
end
...
% for iInter = 1:size(points, 1)-1
    ...
    dists = matGeom.geom2d.distancePoints(points(iInter,:), points(inds, :));
