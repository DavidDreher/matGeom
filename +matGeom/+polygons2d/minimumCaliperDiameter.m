...
while rotated_angle < pi
    ...
    % in the polygon 
    angle_a = matGeom.geom2d.vectorAngle(caliper_a, vector_a);
    angle_b = matGeom.geom2d.vectorAngle(caliper_b, vector_b);
    
    % Determine the smallest of these angles
    minAngle = min(angle_a, angle_b);
    
    % Rotate the calipers by the smallest angle
    caliper_a = matGeom.geom2d.rotateVector(caliper_a, minAngle);
    caliper_b = matGeom.geom2d.rotateVector(caliper_b, minAngle);
    
    rotated_angle = rotated_angle + minAngle;
    
    % compute current width, and update opposite vertex
    if angle_a < angle_b
        line = matGeom.geom2d.createLine(hull(p_a, :), hull(ind_a2, :));
        width = matGeom.geom2d.distancePointLine(hull(p_b, :), line);
        p_a = mod(p_a, nV) + 1;
    
    else
        line = matGeom.geom2d.createLine(hull(p_b, :), hull(ind_b2, :));
        width = matGeom.geom2d.distancePointLine(hull(p_a, :), line);
        p_b = mod(p_b, nV) + 1;
