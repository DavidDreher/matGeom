...
center = matGeom.polygons2d.polygonCentroid(poly);
...
initRes = zeros(8, 1);
for i = 1:8
    line = matGeom.geom2d.createLine([0 0 cos(initAngles(i)) sin(initAngles(i))]);
    rotMat = matGeom.geom2d.createLineReflection(line);
    polyRot = matGeom.geom2d.transformPoint(poly, rotMat);
    initRes(i) = sum(matGeom.polygons2d.distancePointPolygon(poly, polyRot).^2);
end
...
thetaMin = fminbnd(...
    @(theta) sum(matGeom.polygons2d.distancePointPolygon(matGeom.geom2d.transformPoint(poly, matGeom.geom2d.createLineReflection([0 0 cos(theta) sin(theta)])), poly).^2), ...
