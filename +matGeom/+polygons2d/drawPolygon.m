...
ax = gca;
if matGeom.utils.isAxisHandle (px)
    ax          = px;
    px          = varargin{1};
    varargin(1) = [];
end
...
if iscell (px)
    h   = cellfun (@(x)matGeom.polygons2d.drawPolygon (ax, x, varargin{:}), px, 'UniformOutput', 0);
    h   = horzcat (h{:});
    ...
    if any (isnan (px(:)) )
        polygons = matGeom.polygons2d.splitPolygons ([px py]);
        h        = matGeom.polygons2d.drawPolygon (ax, polygons, varargin{:});
