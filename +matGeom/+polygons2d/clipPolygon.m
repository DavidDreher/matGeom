...
if iscell(polygon)
    ...
    for i=1:length(polygon)
        poly2{i} = matGeom.polygons2d.clipPolygon(polygon{i}, w);
    end
    return;
end
...
% clip edges
edges = matGeom.geom2d.clipEdge(edges, w);
...
pc = matGeom.geom2d.centroid(pts);

% sort vertices around polygon
angle = matGeom.geom2d.edgeAngle([repmat(pc, [size(pts, 1) 1]) pts]);
