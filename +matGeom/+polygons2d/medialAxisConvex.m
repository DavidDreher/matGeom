...
rays = zeros(N, 4);
rays(1, 1:4) = matGeom.geom2d.bisector(nodes([2 1 N], :));
rays(N, 1:4) = matGeom.geom2d.bisector(nodes([1 N N-1], :));
for i=2:N-1
    rays(i, 1:4) = matGeom.geom2d.bisector(nodes([i+1, i, i-1], :));
end
...
pint = matGeom.geom2d.intersectLines(rays, rays([2:N 1], :));
...
ti = zeros(N, 1);
for i = 1:N
    line = matGeom.geom2d.createLine(points(mod(i-2, N)+1, :), points(i, :));
    ti(i) = abs(matGeom.geom2d.distancePointLine(pint(i,:), line));
end
...
for i = N+1:2*N-3
    ...
    % create the new ray
    line1 = matGeom.geom2d.createLine(nodes(n1, :), nodes(mod(n1,N)+1, :));
    line2 = matGeom.geom2d.createLine(nodes(mod(n2,N)+1, :), nodes(n2, :));
    ray0 = matGeom.geom2d.bisector(line1, line2);
    ...
    pint = matGeom.geom2d.intersectLines(ray0, rays(ir, :));
    ti = abs(matGeom.geom2d.distancePointLine(pint, line1));
