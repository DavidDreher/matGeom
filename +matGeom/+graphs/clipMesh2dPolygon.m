...
nNodes = size(nodes, 1);
nEdges = size(edges, 1);
nFaces = matGeom.meshes3d.meshFaceNumber(nodes, faces);

% associate each face to the list of its incident edge
faceEdgeInds = matGeom.meshes3d.meshFaceEdges(nodes, edges, faces);


%% Clip the nodes

% find index of nodes inside clipping window
nodeInside = matGeom.polygons2d.isPointInPolygon(nodes, poly);
...
for iEdge = 1:nEdges
    ...
    intersects = matGeom.polygons2d.intersectEdgePolygon(edge0, poly);
...
for iFace = 1:nFaces
    ...
    if all(insideFlags)
        ...
        node0 = nodes2(ind0, :);
        pos0 = matGeom.polygons2d.projPointOnPolygon(node0, poly);
        pos1 = matGeom.polygons2d.projPointOnPolygon(nodes2(nextInd, :), poly);
        sub1 = matGeom.polygons2d.polygonSubcurve(poly, pos0, pos1);
        sub2 = matGeom.polygons2d.polygonSubcurve(poly, pos1, pos0);

        % keep only the smallest polyline
        if matGeom.polygons2d.polylineLength(sub1) < matGeom.polygons2d.polylineLength(sub2)
            sub = sub1;
        else
            sub = sub2;
        end
        ...
        dists = matGeom.geom2d.distancePoints(sub([1 end],:), node0);
        if dists(1) < dists(2)
    ...
    if matGeom.polygons2d.polygonArea(nodes2(newFace, :)) < 0
