...
refNode = matGeom.polygons2d.findPoint([x y], nodes);
...
for n = 1:length(mnodes)
    ...
    neighbors = matGeom.graphs.grAdjacentNodes(edges, node);
...
for n = 1:length(mnodes)
    [nodes, edges] = matGeom.graphs.grRemoveNode(nodes, edges, mnodes(n)-n+1);
end
