...
while n <= length(nodes)
    ...
    p1 = matGeom.polygons2d.findPoint([x-1, y], nodes);
    p2 = matGeom.polygons2d.findPoint([x+1, y], nodes);
    p3 = matGeom.polygons2d.findPoint([x, y-1], nodes);
    p4 = matGeom.polygons2d.findPoint([x, y+1], nodes);
    
    p = [p1 p2 p3 p4];
    p = p(p ~= 0);
        
    if length(p) > 1
        [nodes, edges] = matGeom.graphs.grMergeNodes(nodes, edges, [n p]);
    end
