...
rads = matGeom.geom2d.edgeLength([pts(eds(:,1), :) pts(eds(:,2), :)]) / 2;

% extract middle point of each edge
midPts = matGeom.geom2d.midPoint(pts(eds(:,1), :), pts(eds(:,2), :));

% distance between midpoints and all points
% closest points should be edge vertices
dists = matGeom.geom2d.minDistancePoints(midPts, pts);
