...
bbox = matGeom.polygons2d.polygonBounds(poly);
...
for i = 1:nIter
    ...
    [n, e, f] = matGeom.graphs.boundedVoronoi2d(bbox, germs);
    [n2, e2, f2] = matGeom.graphs.clipMesh2dPolygon(n, e, f, poly); %#ok<ASGLU>
    ...
    for iGerm = 1:nGerms
        polygon = n2(f2{iGerm}, :);
        germs(iGerm,:) = matGeom.polygons2d.polygonCentroid(polygon);
    end
...
function pts = generatePointsInPoly(nPts)
    ...
    while ~isempty(ind)
        ...
        ind = ind(~matGeom.polygons2d.polygonContains(poly, pts(ind, :)));
    end
end
