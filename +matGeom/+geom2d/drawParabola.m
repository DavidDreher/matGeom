...
if matGeom.utils.isAxisHandle(varargin{1})
    ax = varargin{1};
    varargin(1) = [];
else
    ax = gca;
...
for i = 1:length(x0)
    ...
    trans = ...
        matGeom.geom2d.createTranslation(x0(i), y0(i)) * ...
        matGeom.geom2d.createRotation(matGeom.geom2d.deg2rad(theta(i))) * ...
        matGeom.geom2d.createScaling(1, a);
    
	% compute points on the parabola
    [xt, yt] = matGeom.geom2d.transformPoint(t(:), t(:).^2, trans);
