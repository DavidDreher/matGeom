...
for i = 1:nIter
    ...
    inds = matGeom.geom2d.findClosestPoint(points, target);
    corrPoints = target(inds, :);
    
    % compute transform for current iteration
    trans_i = matGeom.geom2d.fitAffineTransform2d(points, corrPoints);

    % apply transform, and update cumulated transform
    points = matGeom.geom2d.transformPoint(points, trans_i);
    trans = trans_i * trans;
end
