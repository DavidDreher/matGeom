...
for i = 1:length(type)    
    if strcmpi(type{i}, 'circle')
        shape{i} = matGeom.geom2d.circleToPolygon(param{i}, 128);
    elseif strcmpi(type{i}, 'rect')
        shape{i} = matGeom.geom2d.rectToPolygon(param{i});
...
if strcmp(option, 'draw')
    for i = 1:length(shape)
        h(i) = matGeom.polygons2d.drawPolygon(shape{i});
    end
else
    for i = 1:length(shape)
        h(i) = matGeom.polygons2d.fillPolygon(shape{i});
    end
end
