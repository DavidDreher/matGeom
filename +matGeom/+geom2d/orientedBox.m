...
while rotatedAngle < pi
    ...
    % in the polygon 
    angleA = matGeom.geom2d.vectorAngle(caliperA, vectorA);
    angleB = matGeom.geom2d.vectorAngle(caliperB, vectorB);
    
    % Determine the smallest of these angles
    angleIncrement = min(angleA, angleB);
    
    % Rotate the calipers by the smallest angle
    caliperA = matGeom.geom2d.rotateVector(caliperA, angleIncrement);
    caliperB = matGeom.geom2d.rotateVector(caliperB, angleIncrement);
    
    rotatedAngle = rotatedAngle + angleIncrement;
    
    % compute current width, and update opposite vertex
    if angleA < angleB
        line = matGeom.geom2d.createLine(hull(indA, :), hull(indA2, :));
        width = matGeom.geom2d.distancePointLine(hull(indB, :), line);
        indA = mod(indA, nV) + 1;
    
    else
        line = matGeom.geom2d.createLine(hull(indB, :), hull(indB2, :));
        width = matGeom.geom2d.distancePointLine(hull(indA, :), line);
        indB = mod(indB, nV) + 1;
...
theta = matGeom.geom2d.rad2deg(minAngle);
