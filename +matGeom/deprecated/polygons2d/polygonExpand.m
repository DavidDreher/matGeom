...
line = zeros(N, 4);
for i=1:N
    side = matGeom.geom2d.createLine(polygon(i,:), polygon(i+1,:));
    %perp = orthogonalLine(side, polygon(i,:));
    %pts2 = pointOnLine(perp, -dist);
    line(i, 1:4) = matGeom.geom2d.parallelLine(side, -dist);
end
...
if dist>0
    ...
    poly2 = zeros(N, 2);
    for i=1:N
        poly2(i,1:2) = matGeom.geom2d.intersectLines(line(i,:), line(i+1,:));
    end
else
    poly2 = polygon;
    % clip polygon with all lines parallel to edges
    for i=1:N
        poly2 = matGeom.polygons2d.clipPolygonHP(poly2, line(i,:));
    end
end
