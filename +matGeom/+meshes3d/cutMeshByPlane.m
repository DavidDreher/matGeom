...
p = inputParser;
addRequired(p,'plane',@matGeom.geom3d.isPlane)
...
VBPl_LI = matGeom.geom3d.isBelowPlane(v, plane);
    ...
    case {1, 2}
        switch part
            case 'above'
                ...
                above = matGeom.meshes3d.removeMeshFaces(v, f, ~(sum(FBP_LI, 2) == 0) );
            case 'in'
                % Faces in the plane, 1 or 2 vertices == 0 -> sum can be 1 or 2
                inside = matGeom.meshes3d.removeMeshFaces(v, f, ~((sum(FBP_LI, 2) > 0 & sum(FBP_LI, 2) < 3)));
            case 'below'
                % Faces below the plane, all three vertices == 1 -> sum has to be 3
                below = matGeom.meshes3d.removeMeshFaces(v, f, ~(sum(FBP_LI, 2) == 3) );
        end
    case {3, 6}
        % Faces above the plane, all three vertices == 0 -> sum has to be 0
        above = matGeom.meshes3d.removeMeshFaces(v, f, ~(sum(FBP_LI, 2) == 0) );
