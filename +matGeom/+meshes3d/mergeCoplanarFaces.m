...
normals = matGeom.meshes3d.faceNormal(nodes, faces);
...
for iFace = 1:nFaces
    ...
    ind = find(matGeom.geom3d.vectorNorm3d(matGeom.geom3d.vectorCross3d(normals(iFace, :), normals)) < acc);
    
    % keep only coplanar faces (test coplanarity of points in both face)
    ind2 = false(size(ind));
    for j = 1:length(ind)
        ind2(j) = matGeom.geom3d.isCoplanar(nodes([faces(iFace,:) faces(ind(j),:)], :), acc);
    end
    ind2 = ind(ind2);
