...
b = matGeom.geom3d.isBelowPlane(nodes, plane);
...
for f = 1:length(faces)
    ...
    poly = nodes(face, :);
    clipped = matGeom.geom3d.clipConvexPolygon3dHP(poly, plane);

    % identify indices of polygon vertices
    inds = zeros(1, size(clipped, 1));
    faceb = face(bf==1); % indices of vertices still in clipped face
    [minDists, I] = matGeom.geom2d.minDistancePoints(nodes(faceb,:), clipped); %#ok<ASGLU>
    for i = 1:length(I)
        inds(I(i)) = faceb(i);
    end
    ...
    [minDists, I] = matGeom.geom2d.minDistancePoints(clipped(indNews, :), nodes2);
...
[sortedNodes, I] = matGeom.geom3d.angleSort3d(nodes2);

% compute normal vector of new face, and reverse order if face points in
% the opposite direction as plane normal
newFaceNormal = matGeom.meshes3d.meshFaceNormals(sortedNodes, 1:length(sortedNodes));
if dot(newFaceNormal, matGeom.geom3d.planeNormal(plane)) < 0
    I(2:end) = I(end:-1:2);
end
