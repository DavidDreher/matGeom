...
pointsCentroid = matGeom.geom2d.centroid(points);

% number of base triangular faces
N = size(faces, 1);

% compute normals of given faces
normals = matGeom.geom3d.planeNormal(matGeom.geom3d.createPlane(...
...
for iFace = 1:N
    ...
    ind = find(abs(matGeom.geom3d.vectorNorm3d(cross(repmat(normals(iFace, :), [N 1]), normals)))<acc);
    ind = ind(ind~=iFace);
    
    % keep only coplanar faces (test coplanarity of points in both face)
    ind2 = iFace;
    for j = 1:length(ind)
        if matGeom.geom3d.isCoplanar(points([faces(iFace,:) faces(ind(j),:)], :), acc)
    ...
    [tmp, I]  = matGeom.geom3d.angleSort3d(points(faceVertices, :)); %#ok<ASGLU>
    ...
    outerNormal = matGeom.meshes3d.meshFaceCentroids(points, face) - pointsCentroid;
    if dot(matGeom.meshes3d.meshFaceNormals(points, face), outerNormal, 2) < 0
        face = face([1 end:-1:2]);
    end
