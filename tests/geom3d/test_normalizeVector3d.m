...
vec1    = [3 0 0];
exp1    = [1 0 0];
norm1   = matGeom.geom3d.normalizeVector3d(vec1);
testCase.assertEqual(exp1, norm1);

vec2    = [0 3 0];
exp2    = [0 1 0];
norm2   = matGeom.geom3d.normalizeVector3d(vec2);
testCase.assertEqual(exp2, norm2);

vec3    = [0 0 3];
exp3    = [0 0 1];
norm3   = matGeom.geom3d.normalizeVector3d(vec3);
testCase.assertEqual(exp3, norm3);

function testDiagoVector(testCase)

vec = [6 8 0];
norm = matGeom.geom3d.normalizeVector3d(vec);
exp = [3/5 4/5 0];
...
norm = matGeom.geom3d.normalizeVector3d(vecs);
testCase.assertEqual(exp, norm);
