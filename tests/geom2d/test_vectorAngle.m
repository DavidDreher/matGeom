...
ang = matGeom.geom2d.vectorAngle([1 0]);
testCase.assertEqual(0, ang, 'AbsTol', .01);

ang = matGeom.geom2d.vectorAngle([0 1]);
...
ang = matGeom.geom2d.vectorAngle([-1 1]);
testCase.assertEqual(3*pi/4, ang, 'AbsTol', .01);

function testSinglePiCentered(testCase)

ang = matGeom.geom2d.vectorAngle([1 0], pi);
testCase.assertEqual(0, ang, 'AbsTol', .01);

ang = matGeom.geom2d.vectorAngle([0 1], pi);
testCase.assertEqual(pi/2, ang, 'AbsTol', .01);
...
ang = matGeom.geom2d.vectorAngle([-1 1], pi);
testCase.assertEqual(3*pi/4, ang, 'AbsTol', .01);
...
testCase.assertEqual(angs, matGeom.geom2d.vectorAngle(vecs), 'AbsTol', .01);

function testArrayPiCentered(testCase)

vecs = [1 0;0 1;-1 0;0 -1;1 1];
angs = [0;pi/2;pi;3*pi/2;pi/4];
testCase.assertEqual(angs, matGeom.geom2d.vectorAngle(vecs, pi), 'AbsTol', .01);

function testSingleZeroCentered(testCase)

ang = matGeom.geom2d.vectorAngle([1 0], 0);
testCase.assertEqual(0, ang, 'AbsTol', .01);

ang = matGeom.geom2d.vectorAngle([0 1], 0);
testCase.assertEqual(pi/2, ang, 'AbsTol', .01);

ang = matGeom.geom2d.vectorAngle([0 -1], 0);
testCase.assertEqual(-pi/2, ang, 'AbsTol', .01);

ang = matGeom.geom2d.vectorAngle([-1 1], 0);
testCase.assertEqual(3*pi/4, ang, 'AbsTol', .01);
...
testCase.assertEqual(angs, matGeom.geom2d.vectorAngle(vecs, 0), 'AbsTol', .01);
...
v2 = [0 1];
ang = pi /2 ;
testCase.assertEqual(ang, matGeom.geom2d.vectorAngle(v1, v2), 'AbsTol', .01);
